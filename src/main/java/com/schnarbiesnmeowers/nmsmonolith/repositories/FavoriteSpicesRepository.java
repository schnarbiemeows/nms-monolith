...
public interface FavoriteSpicesRepository extends JpaRepository<FavoriteSpices, Integer> {
    ...
    @Query(value = "select * from favorite_spices fs where fs.user_id = ?1 and fs.is_local=true and fs.spices_id=?2 and fs.actv='Y'", nativeQuery = true)
    public Iterable<FavoriteSpices> findLocalSpicesInAnyUserFavorites(int userId, int spiceId);

    @Query(value = "select * from favorite_spices fs where fs.is_local=false and fs.spices_id=?1 and fs.actv='Y'", nativeQuery = true)
